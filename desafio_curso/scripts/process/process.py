...
df_clientes = spark.sql("SELECT * FROM desafio_curso.tbl_clientes").distinct()
df_divisao = spark.sql("SELECT * FROM desafio_curso.tbl_divisao").distinct()
df_regiao = spark.sql("SELECT * FROM desafio_curso.tbl_regiao").distinct()
df_vendas = spark.sql("SELECT * FROM desafio_curso.tbl_vendas").distinct()
df_endereco = spark.sql("SELECT * FROM desafio_curso.tbl_endereco").distinct()
...
df_stage = df_stage.withColumn("date_key", when(col("date_key")=="", "00/00/0000").when(col("date_key")=="null", "00/00/0000").otherwise(df_stage.date_key))

#COLUNA DISCOUNT_AMOUNT
df_stage = df_stage.withColumn("discount_amount", trim(col("discount_amount")))
df_stage = df_stage.withColumn("discount_amount", when(col("discount_amount")=="", "0,00").when(col("discount_amount")=="null", "0,00").otherwise(df_stage.discount_amount))
...
df_stage = df_stage.withColumn("list_price", when(col("list_price")=="", "0,00").when(col("list_price")=="null", "0,00").otherwise(df_stage.list_price))
...
df_stage = df_stage.withColumn("promised_delivery_date", when(col("promised_delivery_date")=="", "00/00/0000").when(col("promised_delivery_date")=="null", "00/00/0000").otherwise(df_stage.promised_delivery_date))

#COLUNA SALES_AMOUNT
df_stage = df_stage.withColumn("sales_amount", trim(col("sales_amount")))
df_stage = df_stage.withColumn("sales_amount", when(col("sales_amount")=="", "0,00").when(col("sales_amount")=="null", "0,00").otherwise(df_stage.sales_amount))
...
df_stage = df_stage.withColumn("sales_amount_based_on_list_price", when(col("sales_amount_based_on_list_price")=="", "0,00").when(col("sales_amount_based_on_list_price")=="null", "0,00").otherwise(df_stage.sales_amount_based_on_list_price))
...
df_stage = df_stage.withColumn("sales_cost_amount", when(col("sales_cost_amount")=="", "0,00").when(col("sales_cost_amount")=="null", "0,00").otherwise(df_stage.sales_cost_amount))
...
df_stage = df_stage.withColumn("sales_margin_amount", when(col("sales_margin_amount")=="", "0,00").when(col("sales_margin_amount")=="null", "0,00").otherwise(df_stage.sales_margin_amount))
...
df_stage = df_stage.withColumn("sales_price", when(col("sales_price")=="", "0,00").when(col("sales_price")=="null", "0,00").otherwise(df_stage.sales_price))
...
df_stage = df_stage.withColumn('pk_tempo', col('date_key'))


# In[40]:


df_stage = df_stage.withColumn('pk_cliente', col('customer_key'))


# In[42]:


df_stage = df_stage.withColumn('pk_local', col('address_number'))


# In[43]:


df_stage = df_stage.withColumn('pk_pedido', col('order_number'))
...
dim_local = df_stage.select("pk_local","city", "country", "state", "region_code", "region_name").distinct()


# In[47]:


fato_vendas = df_stage.select("pk_pedido","order_number", "invoice_number", "customer_key", "sales_rep", "item_number", "item", "item_class", "line_number", "list_price", "date_key", "promised_delivery_date", "actual_delivery_date", "region_code", "sales_quantity", "sales_price", "discount_amount", "sales_amount", "sales_amount_based_on_list_price", "sales_cost_amount").distinct()


# In[48]:


dim_tempo = df_stage.select("pk_tempo","date_key", "invoice_date", "invoice_day", "invoice_month", "invoice_year", "quarter").distinct()


# In[49]:


dim_cliente = df_stage.select("pk_cliente","customer_key", "customer", "customer_type", "phone", "address_number", "customer_address_1", "customer_address_2", "customer_address_3", "customer_address_4", "division", "division_name", "line_of_business", "business_family", "business_unit", "regional_sales_mgr", "search_type", "zip_code").distinct()


# In[50]:


#Reforçando a extração de duplicidades


# In[51]:


dim_local=dim_local.dropDuplicates()
...
fato_vendas.select('order_number').distinct().count()
...
fato_vendas.select(sum("sales_amount")).show()
